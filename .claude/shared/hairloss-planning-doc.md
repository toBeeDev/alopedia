...
- **검증 로직**:
  ...
  - 스크린샷/캡처 이미지 차단
- **업로드 UX**: BrownFeed 스타일 프로그레스바 + "페이지 이동 금지" 안내
...
| Animation  | Framer Motion                     | BrownFeed 스타일의 인터랙션     |
...
CREATE TABLE profiles (
  ...
  nickname TEXT UNIQUE NOT NULL,       -- "두피전사_384721"
  level INTEGER DEFAULT 1,
  exp INTEGER DEFAULT 0,
  streak_current INTEGER DEFAULT 0,
  streak_best INTEGER DEFAULT 0,
  avatar_seed TEXT,                     -- 익명 아바타 생성 시드
...
CREATE TABLE scans (
  ...
  created_at TIMESTAMPTZ DEFAULT now(),
  images JSONB NOT NULL,               -- [{type: "top"|"front"|"side", url, thumbnail_url}]
...
CREATE TABLE analyses (
  ...
  score DECIMAL(4,1),                  -- 0.0 ~ 100.0
  details JSONB NOT NULL,              -- {hairline, density, thickness, scalp_condition, advice}
...
CREATE TABLE treatments (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID REFERENCES auth.users NOT NULL,
  type TEXT NOT NULL,                  -- medication | procedure | supplement | shampoo
  name TEXT NOT NULL,                  -- "피나스테리드 1mg"
  started_at DATE,
  ended_at DATE,
  dosage TEXT,
  notes TEXT,
...
CREATE TABLE posts (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID REFERENCES auth.users NOT NULL,
  board TEXT NOT NULL,                 -- medication_review | procedure_review | qna | lounge
  title TEXT NOT NULL,
  content TEXT NOT NULL,
  tags TEXT[],
  images JSONB,
  scan_id UUID REFERENCES scans,       -- Before/After 연결
  vote_count INTEGER DEFAULT 0,
  comment_count INTEGER DEFAULT 0,
  is_adopted BOOLEAN DEFAULT false,    -- Q&A 채택
  created_at TIMESTAMPTZ DEFAULT now()
);
...
CREATE TABLE comments (
  ...
  user_id UUID REFERENCES auth.users NOT NULL,
  parent_id UUID REFERENCES comments,  -- 대댓글
  content TEXT NOT NULL,
  vote_count INTEGER DEFAULT 0,
  created_at TIMESTAMPTZ DEFAULT now()
);
...
CREATE TABLE votes (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID REFERENCES auth.users NOT NULL,
  target_type TEXT NOT NULL,           -- post | comment | scan
  target_id UUID NOT NULL,
  vote_type TEXT NOT NULL,             -- up | empathy
  created_at TIMESTAMPTZ DEFAULT now(),
...
CREATE TABLE achievements (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID REFERENCES auth.users NOT NULL,
  badge_code TEXT NOT NULL,            -- first_scan | streak_7 | streak_30 | review_king
...
Scans:
  POST   /api/scans                    ← 촬영 세션 생성 + 이미지 업로드
  GET    /api/scans                    ← 내 촬영 히스토리
  GET    /api/scans/:id                ← 촬영 상세
  DELETE /api/scans/:id                ← 촬영 삭제
  PATCH  /api/scans/:id/visibility     ← 공개/비공개 전환

Analysis:
  POST   /api/scans/:id/analyze        ← AI 분석 트리거
  GET    /api/scans/:id/analysis       ← 분석 결과 조회

Dashboard:
  GET    /api/dashboard/timeline       ← 타임라인 데이터
  GET    /api/dashboard/compare        ← Before/After 비교
  GET    /api/dashboard/calendar       ← 캘린더 (스트릭)
  GET    /api/dashboard/report         ← PDF 리포트 생성

Treatments:
  POST   /api/treatments               ← 치료 기록 추가
  GET    /api/treatments               ← 내 치료 목록
  PATCH  /api/treatments/:id           ← 치료 수정
  DELETE /api/treatments/:id           ← 치료 삭제

Posts:
  GET    /api/posts?board=             ← 게시판 목록
  POST   /api/posts                    ← 글 작성
  GET    /api/posts/:id                ← 글 상세
  PATCH  /api/posts/:id                ← 글 수정
  DELETE /api/posts/:id                ← 글 삭제
  POST   /api/posts/:id/vote           ← 추천/공감
...
Feed:
  GET    /api/feed                     ← 메인 피드 (공개 분석 카드)
  GET    /api/feed/best                ← 명예의전당

Profile:
  GET    /api/profile                  ← 내 프로필
  PATCH  /api/profile                  ← 프로필 수정
  GET    /api/profile/achievements     ← 내 뱃지
  GET    /api/profile/stats            ← 활동 통계
```
...
- → 상세: `agents/security-reviewer.agent.md` 참조
...
| 주차 | 태스크                              | 관련 에이전트                                 |
| ---- | ----------------------------------- | --------------------------------------------- |
| W1   | 프로젝트 셋업, DB 스키마, Auth 연동 | orchestrator, frontend-dev, security-reviewer |
| W2   | 카메라 촬영 모듈 + 이미지 업로드    | frontend-dev, code-reviewer                   |
| W2   | Gemini API 연동 + 분석 파이프라인   | frontend-dev, security-reviewer               |
| W3   | 분석 결과 카드 UI + 히스토리 뷰     | frontend-dev, ux-writer                       |
| W3   | 업로드 보안 검증 + EXIF 스트리핑    | security-reviewer, unit-tester                |
| W4   | E2E 테스트 + 버그픽스 + 카피 검수   | unit-tester, ux-writer, code-reviewer         |
| W4   | **MVP 릴리즈 게이트**               | orchestrator (전 에이전트 승인)               |
...
```
PRD.md (본 문서)
  ├── agents/orchestrator.agent.md      ← 워크플로우, 의사결정
  ├── agents/frontend-dev.agent.md      ← 컴포넌트 구현 스펙
  ├── agents/code-reviewer.agent.md     ← 리뷰 체크리스트
  ├── agents/security-reviewer.agent.md ← 보안 정책 상세
  ├── agents/unit-tester.agent.md       ← 테스트 전략
  ├── agents/ux-writer.agent.md         ← 카피 가이드라인
  ├── shared/glossary.md                ← 용어 정의
  └── shared/conventions.md             ← 코딩 컨벤션
```
